...
;定义通用选择
(define (real-part z)
  (cond ((eq? (type-tag z) 'rect) real-part-rect z)
        ((eq? (type-tag z) 'pola) real-part-pola z)
        (else (error "Unknown type " z))
  )
)

(define (imag-part z)
  (cond ((eq? (type-tag z) 'rect) imag-part-rect z)
        ((eq? (type-tag z) 'pola) imag-part-pola z)
        (else (error "Unknown type " z))
  )
)

(define (magnitude z)
  (cond ((eq? (type-tag z) 'rect) magnitude-rect z)
        ((eq? (type-tag z) 'pola) magnitude-pola z)
        (else (error "Unknown type " z))
  )
)

(define (angle z)
  (cond ((eq? (type-tag z) 'rect) angle-rect z)
        ((eq? (type-tag z) 'pola) angle-pola z)
        (else (error "Unknown type " z))
  )
)
