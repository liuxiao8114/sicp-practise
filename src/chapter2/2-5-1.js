...
Express.TYPE_TABLE = key => ({
  [key]: {
    SchemeNumber: SchemeNumber[key],
    Rational: Rational[key],
    Complex: Complex[key],
  }
})

const caculator = op => (x, y) => {
  let exp = null
...
const add = caculator('add')
const sub = caculator('sub')
const mul = caculator('mul')
const div = caculator('div')
...
SchemeNumber.prototype = {
  constructor: SchemeNumber,
  toString() {
    return this.num
  },
  convertToParent() {
